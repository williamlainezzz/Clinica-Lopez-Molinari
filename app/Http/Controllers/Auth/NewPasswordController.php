...
{
    // Email que viene en el enlace (token)
    $email = $request->email;
    ...
    /**
     * Maneja el POST del formulario de nueva contraseña.
     * 1) Valida token, email y nueva contraseña
     * 2) Busca el usuario por email
     * 3) Actualiza la contraseña en tu campo PWD_USUARIO
     * 4) Elimina el token
    ...
    public function store(Request $request)
{
    ...
    // --- (1) Si hay pregunta, validar primero como ya hiciste ---
    ...
    // --- (2) Validar TOKEN de reset manualmente ---
    ...
    // El token en la tabla suele estar HASHeado por Laravel → usar Hash::check
    if (!Hash::check($request->token, $record->token)) {
        return back()->withErrors(['email' => 'El enlace no es válido o ya fue utilizado.'])->withInput();
    }

    // Verificar caducidad (usa el "expire" del broker 'users' en config/auth.php, por defecto 60 min)
    ...
    // --- (4) Actualizar contraseña en tu campo PWD_USUARIO ---
    $user->PWD_USUARIO = Hash::make($request->password);
    $user->setRememberToken(Str::random(60));
    $user->save();

    // (Opcional) cerrar sesiones antiguas si usas driver "database":
    // DB::table('sessions')->where('user_id', $user->COD_USUARIO)->delete();

    // --- (5) Invalidar el token (borrar fila) ---
...
// Helper de normalización (si no lo tienes ya en este controlador)
