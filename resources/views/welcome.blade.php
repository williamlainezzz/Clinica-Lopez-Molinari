...
<body class="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(29,78,216,0.12),_transparent_28%),radial-gradient(circle_at_bottom_right,_rgba(59,130,246,0.14),_transparent_34%),linear-gradient(180deg,_#f8fbff_0%,_#eef4ff_48%,_#f8fbff_100%)] text-slate-800 relative">
  @php
    $preguntasSeg = $preguntasSeg ?? \App\Models\PreguntaSeguridad::where('ESTADO', 1)->orderBy('TEXTO_PREGUNTA')->get();
  @endphp
  ...
  >
    ...
    <div x-cloak x-show="showRegister" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div x-transition class="modal-panel modal-card relative w-full max-w-5xl overflow-hidden">
        ...
        <div
          x-data="{
            step: {{ $errors->register->hasAny(['PREGUNTA_1','PREGUNTA_2','RESPUESTA_1','RESPUESTA_2','password','password_confirmation']) ? 2 : 1 }},
            totalSteps: 2,
        ...
        >
          <div class="border-b border-slate-200 bg-gradient-to-r from-slate-50 to-white px-6 py-5">
            ...
            <div class="mt-5 flex items-center gap-3">
              <div class="flex flex-1 items-center gap-3">
                <div class="flex h-9 w-9 items-center justify-center rounded-full text-sm font-bold transition" :class="step >= 1 ? 'bg-blue-700 text-white' : 'bg-slate-200 text-slate-500'">1</div>
                <div class="h-1 flex-1 rounded-full" :class="step >= 2 ? 'bg-blue-600' : 'bg-slate-200'"></div>
                <div class="flex h-9 w-9 items-center justify-center rounded-full text-sm font-bold transition" :class="step >= 2 ? 'bg-blue-700 text-white' : 'bg-slate-200 text-slate-500'">2</div>
              </div>
              <span class="rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700" x-text="step === 1 ? 'Datos personales' : 'Seguridad y acceso'"></span>
            </div>
          </div>
          ...
          <div class="max-h-[85vh] overflow-y-auto p-6">
            <form method="POST" action="{{ route('register') }}" novalidate class="space-y-6">
              ...
              <section x-show="step === 1" x-transition.opacity.duration.150ms class="rounded-3xl border border-slate-200 bg-white px-5 py-5 shadow-sm">
                <div class="mb-4">
                  <p class="section-kicker">Paso 1</p>
                  <h4 class="mt-2 text-lg font-bold text-slate-900">Datos personales</h4>
                  <p class="mt-1 text-sm text-slate-600">Te pediremos solo los datos necesarios para crear tu acceso de forma rapida.</p>
                </div>

                <div class="grid gap-4 md:grid-cols-2">
                  <div>
                    <x-input-label for="NOMBRES_COMPLETOS" :value="__('Nombres')" />
                    <x-text-input id="NOMBRES_COMPLETOS" class="block mt-1 w-full" type="text" :value="trim(collect([old('PRIMER_NOMBRE'), old('SEGUNDO_NOMBRE')])->filter()->implode(' '))" required autofocus />
                  ...
                  <div>
                    ...
                    <x-text-input id="APELLIDOS_COMPLETOS" class="block mt-1 w-full" type="text" :value="trim(collect([old('PRIMER_APELLIDO'), old('SEGUNDO_APELLIDO')])->filter()->implode(' '))" required />
              ...
              <section x-show="step === 2" x-transition.opacity.duration.150ms class="rounded-3xl border border-slate-200 bg-white px-5 py-5 shadow-sm">
                <div class="mb-4">
                  <p class="section-kicker">Paso 2</p>
                  <h4 class="mt-2 text-lg font-bold text-slate-900">Seguridad de la cuenta</h4>
                  <p class="mt-1 text-sm text-slate-600">Configura tus preguntas de seguridad y una contrasena segura para terminar el registro.</p>
                </div>
                ...
                <div
                  ...
                  class="mt-5 grid gap-4 md:grid-cols-[minmax(0,1.15fr)_minmax(280px,0.85fr)]"
                >
                  <div class="space-y-4">
                    <div>
                      <x-input-label for="password" :value="__('Contrasena')" />
              ...
              <div class="sticky bottom-0 z-10 -mx-6 -mb-6 mt-6 border-t border-slate-200 bg-white/95 px-6 py-4 backdrop-blur">
                <div class="flex items-center justify-between gap-3">
                  <button type="button" class="text-sm font-medium text-slate-600 transition hover:text-slate-900" @click="step === 1 ? showRegister = false : prev()" x-text="step === 1 ? 'Cancelar' : 'Volver'"></button>

                  <div class="flex items-center gap-3">
                    <button type="button" class="inline-flex items-center justify-center rounded-2xl bg-blue-700 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-800" x-show="step === 1" @click="next()">
                      Continuar
                    </button>

                    <x-primary-button x-show="step === 2" class="px-6">
                      Crear cuenta
                    </x-primary-button>
                  </div>
                </div>
    ...
    (function () {
      ...
      function bindRegisterFields() {
        ...
        function syncHiddenFields() {
          ...
          if ($preview) {
            $preview.textContent = ($fullNames.value.trim() || $fullSurnames.value.trim())
        ...
        ['input', 'change'].forEach((eventName) => {
          $fullNames.addEventListener(eventName, syncHiddenFields);
          $fullSurnames.addEventListener(eventName, syncHiddenFields);
        });
